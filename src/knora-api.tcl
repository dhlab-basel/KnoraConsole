namespace eval knoraApi {
   ...
   proc get_users { {user_iri {}} } {
      ...
      set url $knora_server/admin/users
      if { $user_iri != {} } {
         set url $url/[::helper::uencode $user_iri]
      }
      catch { rest::get $url {} $options } res error
      if { [dict get $error "-code"] != 0} {
         return [dict create errormsg $res]
      }
      ...
      set res [json::json2dict $res]
      if { $user_iri != {} } {
         return [dict get $res user]
      } else {
         return [dict get $res users]
      }
   }
   #===============================================================================
